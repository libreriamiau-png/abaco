...
function normalizarCantidad(libroOValor, valorPorDefecto = 0) {
  const valor = (typeof libroOValor === 'object' && libroOValor !== null)
    ? libroOValor.cantidad
    : libroOValor;
...
function esLibroActivo(libro) {
  return normalizarCantidad(libro, 0) > 0;
}

function esLibroArchivado(libro) {
  return normalizarCantidad(libro, 0) === 0;
}

function puedeEditarLibro(libro) {
  return esLibroActivo(libro);
}

function puedeExportarseComercialmente(libro) {
  return esLibroActivo(libro);
}
...
async function abrirFichaSegunEstado(idOLibro, modo = 'lectura') {
  const libro = (typeof idOLibro === 'object' && idOLibro !== null)
    ? idOLibro
    : await obtenerLibroPorId(idOLibro);
  ...
  if (modo === 'editar') {
    if (puedeEditarLibro(libro)) {
      return editarLibro(libro.id);
    }

    toast('Ficha archivada: solo lectura');
  }

  return verDetalle(libro);
}

function tieneFiltroCantidadExplicito() {
  const inputCantidad = document.getElementById('bCantidad');
  return !!(inputCantidad && inputCantidad.value.trim() !== '');
}

function incluirArchivadosEnBusqueda() {
  const checkbox = document.getElementById('bIncluirArchivados');
  return !!(checkbox && checkbox.checked);
}

function filtrarLibrosExportables(libros) {
  return (libros || []).filter(puedeExportarseComercialmente);
}
...
async function cargarInventario() {
  ...
  try {
    const filtros = ['cantidad=gt.0'];

    if (busquedaActual) {
      filtros.push(`or=(titulo.ilike.*${encodeURIComponent(busquedaActual)}*,autor.ilike.*${encodeURIComponent(busquedaActual)}*,isbn.ilike.*${encodeURIComponent(busquedaActual)}*)`);
    }

    const query = filtros.join('&');
    totalLibros = await apiConteo(`/inventario?${query}`);
    
    const desde = pagina * POR_PAGINA;
    const hasta = desde + POR_PAGINA - 1;
    
    const response = await fetch(`${CONFIG.SUPABASE_URL}/inventario?${query}&order=id.desc&offset=${desde}&limit=${POR_PAGINA}`, {
      headers: {
        ...API_HEADERS,
        'Range': `${desde}-${hasta}`,
        'Prefer': 'count=exact'
...
function toggleSeleccionTodos() {
  ...
  if (checkboxAll.checked) {
    ...
    if (confirm(`¿Seleccionar TODOS los ${totalLibros} libros del inventario activo?

✓ SÍ → Selecciona todo el inventario activo
...
async function actualizarDashboard() {
  try {
    ...
    const valor = (todos || []).reduce((s, l) => s + parseFloat(l.precio || 0) * normalizarCantidad(l, 1), 0);
...
async function guardarLibro() {
  ...
  try {
    ...
    const payload = {
      ...
      cantidad: document.getElementById('fCantidad').value === '' ? 1 
      : parseInt(document.getElementById('fCantidad').value, 10),      
...
async function verDetalle(idOLibro) {
  ...
  const html = `
    ...
    ${esLibroArchivado(libro) ? `<div style="display:inline-block; margin:8px 0 4px; padding:4px 10px; border-radius:999px; background:#fff3cd; color:#856404; font-weight:700; font-size:.85rem;">Archivado · sin stock</div>` : ''}
    ...
    ${libro.localiz || libro.cantidad === 0 || libro.cantidad ? `
      <div class="detalle-seccion">
        ${libro.localiz ? `<strong>Localización:</strong> ${escaparHTML(libro.localiz)}<br>` : ''}
        ${libro.cantidad === 0 || libro.cantidad ? `<strong>Cantidad:</strong> ${libro.cantidad}<br>` : ''}
...
function limpiarBusquedaAvanzada() {
  return limpiarBusquedaCompleta();
}
...
function construirQueryBusqueda() {
  ...
  if (!tieneFiltroCantidadExplicito()) {
    filtros.push(incluirArchivadosEnBusqueda() ? 'cantidad=gte.0' : 'cantidad=gt.0');
  }
...
function mostrarResultados(libros) {
  ...
  libros.forEach(libro => {
    ...
    item.dataset.estadoInventario = esLibroArchivado(libro) ? 'archivado' : 'activo';
    ...
    if (esLibroArchivado(libro)) partes.push('Archivado · sin stock');
    ...
    if (puedeEditarLibro(libro)) {
      ...
      btnEditar.onclick = (e) => {
        e.stopPropagation();
        abrirFichaSegunEstado(libro, 'editar');
      };
...
function limpiarBusquedaCompleta() {
  ...
  const checkboxArchivados = document.getElementById('bIncluirArchivados');
  if (checkboxArchivados) checkboxArchivados.checked = false;
...
async function obtenerTodosLosLibros(filtroExtra = '') {
  ...
  while (true) {
    const filtro = filtroExtra ? `&${filtroExtra}` : '';
    const batch = await apiFetch(`/inventario?select=*${filtro}&limit=${limit}&offset=${offset}&order=id.desc`);
...
async function exportarAbeBooks() {
  try {
    ...
    if (seleccion.seleccionTotal) {
      ...
      librosParaExportar = await obtenerTodosLosLibros('cantidad=gt.0');
    } else {
      // Obtener solo los libros seleccionados y filtrar agotados
      librosParaExportar = filtrarLibrosExportables(await obtenerLibrosPorIds(seleccion.ids));
    }

    if (!librosParaExportar.length) {
      throw new Error('No hay libros exportables: las fichas archivadas (cantidad = 0) quedan excluidas');
    }
...
function generarCSVAbeBooks(libros) {
  ...
  const exportables = filtrarLibrosExportables(libros);
  const rows = exportables.map(libro => [
    libro.stock_no || '',
    libro.tipo || 'Libro',
